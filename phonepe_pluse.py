...
def transaction_aggregate(path):
    for state in os.listdir(path):
        for year in os.listdir(path + '\\' + state + '\\'):
            for quarter in os.listdir(path + '\\' + state + '\\' + year + '\\'):
                df = open(path + '\\' + state + '\\' + year + '\\' + quarter, "r")
                df2 = json.load(df)
                quarter_divi = quarter.replace('.json','')
                with get_db_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("select count(*) from aggregate_transaction where state = %s and year = %s and quarter = %s",
                                   (state, year, quarter))
                            ...
                            for rec in data['paymentInstruments']:
                                if count == 0:
                                    cursor.execute(
                                        ...
                                        (state, year, quarter_divi, data['name'], rec['amount'], rec['count']))
                                else:
                                    cursor.execute(
    ...
    for state in os.listdir(path):
        for year in os.listdir(path + '/' + state + '/'):
            for quarter in os.listdir(path + '/' + state + '/' + year + '/'):
                ...
                with get_db_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("select count(*) from aggregate_user where state = %s and year = %s and quarter = %s",
                                   (state, year, quarter))
                    count = cursor.fetchone()[0]
                    try:
                        for data in df2['data']['usersByDevice']:
                            if count == 0:
                                cursor.execute("insert into aggregate_user(state,year,quarter,brand,count)values(%s,%s,%s,%s,%s)",
                                               (state,year,quarter_divi,data['brand'],data['count']))
                            else:
                                cursor.execute("update aggregate_user set state = %s, year = %s, quarter = %s, brand = %s, count = %s",
                                               (state,year,quarter_divi,data['brand'],data['count']))
                            conn.commit()
                    except Exception as e:
                        print(e)
                            ...
                            for rec in data['metric']:
                                if count == 0:
                                    cursor.execute(
                                        ...
                                        (state, year, quarter_divi, rec['amount'], rec['count'],data['name']))
                                else:
                                    cursor.execute(
                        ...
                        for record in df2['data']['hoverData'].items():
                            ...
                            if count == 0:
                                cursor.execute(
                                    ...
                                    (state, year, quarter_divi, usercount, district))
                            else:
                                cursor.execute(
                ...
                with get_db_connection() as conn:
                    ...
                    try:
                        for record in df2['data']['districts']:
                            print(record['entityName'],record['metric']['amount'])
                            district = record[0].replace('district','')
                            usercount = record[1]['registeredUsers']
                            if count == 0:
                                cursor.execute(
                                    "insert into top_transactions(state,year,quarter,amount,count,district)values(%s,%s,%s,%s,%s,%s)",
                                    (state, year, quarter_divi, record['metric']['amount'],record['metric']['count'], record['entityName']))
                            else:
                                cursor.execute(
                        ...
                        for record in df2['data']['districts']:
                            if count == 0:
                                cursor.execute(
                                    ...
                                    (state, year, quarter_divi, record['registeredUsers'], record['name']))
                            else:
                                cursor.execute(
...
def main():
    ...
    pages = st.sidebar.selectbox('NAVIGATION',['HOME', 'VISUALIZATION'])
    if pages == 'HOME':
        st.header("PHONEPE PLUSE DATA VISUALIZATION")
        st.divider()
